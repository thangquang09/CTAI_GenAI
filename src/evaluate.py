...
def run_chunking_pipeline(
    ...
    chunks_file = os.path.join(
        output_dir, f"chunks_{strategy}_{chunk_size}_{overlap}.jsonl"
    )
...
def detect_metadata_key(
    ...
    if "metadata" in payload and "article_id" in payload.get("metadata", {}):
        ...
        raise ValueError(
            f"Cannot find article_id in payload structure: {payload.keys()}"
        )
    ...
    for article_id in tqdm(article_ids, desc="Building gold embeddings"):
        ...
        results = client.scroll(
            ...
            scroll_filter=Filter(
                must=[
                    FieldCondition(key=metadata_key, match=MatchValue(value=article_id))
                ]
            ),
...
def evaluate_chunk_level_semantic(
    ...
    for item in tqdm(eval_data, desc="Evaluating queries"):
        ...
        gold_vecs = [
            gold_embeddings.get(aid) for aid in gold_articles if aid in gold_embeddings
        ]
            ...
            try:
                ...
                if chunk_id:
                    ...
                    if point and len(point) > 0:
                        ...
                        chunk_vec = np.array(
                            vectorstore.embeddings.embed_query(chunk_text)
                        )
                else:
                ...
                max_sim = max(
                    cosine_similarity(chunk_vec, gold_vec) for gold_vec in gold_vecs
                )

                # Update debug stats
                debug_stats["max_similarity"] = max(
                    debug_stats["max_similarity"], max_sim
                )
                debug_stats["min_similarity"] = min(
                    debug_stats["min_similarity"], max_sim
                )
    ...
    print(
        f"  Re-embedded chunks: {debug_stats['total_chunks'] - debug_stats['retrieved_vectors']}"
    )
    if debug_stats["retrieved_vectors"] > 0:
        similarities = debug_stats["similarity_distribution"]
        print(
            f"  Similarity range: [{debug_stats['min_similarity']:.4f}, {debug_stats['max_similarity']:.4f}]"
        )
        ...
        above_threshold = sum(1 for s in similarities if s >= cosine_threshold) / len(
            similarities
        )
